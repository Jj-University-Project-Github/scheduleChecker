...
def menu(menus):
    ...
    if value.lower() == 'n': #만약 입력받은 번호가 n 또는 N일 경우
        ...
        value = int(value)
        return menus[value - 1]["run"](menus[value - 1]["query"]) #입력된 번호의 내용을 매개변수와 함께 실행하기
...
def showData(data):
    ...
    if data is None: #만약 데이터가 없을 경우
        ...
        for i in data: #과목 데이터를 불러와서 i에 대입하기
            print("과목 : %s\n시간 : %s\n학관 : %s\n관실번호 : %s" %
                  (i["name"], i["time"], i["at"], i["number"])) #n번째 강의의 과목, 시간, 학관, 관실번호를 출력.
            for j in i["professorInfo"]: #n번째 강의의 교수님 정보를 j에 대입
                print("성함 : %s\n학관 : %s\n랩실 : %s\n내선번호 : 063-220-%s\n메일주소 : %s@jj.ac.kr\n" %
